...
#spaceship class. the class of all spaceships in the game
...
#class for the player fired lasers
...
#class for the enemy fired lasers
...
#star class, allowing for generation, storage of information,
#and carrying on of processes related to the stars in the background 
class Star(object):
...
def appStarted(app, r=1,stars = [], lasersFired = 0, hits = 0, lives = 5):
    app.timerDelay = 1
    app.gameOver = False
    app.victory = False
    app.proceed = False
    ...
    app.fireCount = 1
    app.displayExplosionUntil = None
...
def generateNewEnemies(app):
    ...
    if app.round != 7:
        ...
        cx = app.width//2
        cy = app.width//10
        app.enemies.append(bossShip(cx, cy,app.margin/15, 5, 
                                             'Boss'))

#designates the enmies the enemies that are allowed to fire
...
def timerFired(app):
    if not app.gameOver and not app.victory:
        if (app.enemyLasers != [] or app.enemies != []
            ...
            #coded with aid of pseudocode and explanation from:
...
#carries out all the processes of the bomb            
def bombProcesses(app):
    if (app.round == 7 and app.enemies != [] 
        and app.fires%3000 == 0 
        and app.enemies[0].chargingBomb == False
        and app.bomb == []):
        app.enemies[0].chargingBomb = True
    if app.fires%2000 and app.round == 7:
        if app.enemies != [] and app.enemies[0].chargingBomb == True:
            chargeBomb(app)
            if (app.bomb[0].r == 5
                and app.tempFire != None 
                and app.fires - app.tempFire == 1000):
                app.fireBomb = True
    if app.fireBomb == True:
        moveBomb(app)
    explodeBomb(app,0)
    deleteBomb(app)
    if app.bomb != [] and app.fires == app.displayExplosionUntil:
        app.bomb.pop()
        app.displayExplosionUntil = None
...
def deleteBomb(app):
    if (app.bomb != [] and app.bomb[0].r == 100 
        and app.displayExplosionUntil == None):
        bomb = app.bomb[0]
        for enemy in app.enemies:
            bossAffected = (abs(enemy.cx-bomb.cx) < bomb.r+enemy.r
                            and abs(enemy.cy-bomb.cy) < bomb.r+enemy.r)
            if bossAffected and app.fireBomb == True:
                hurtBoss(app,enemy,1)
            elif bossAffected and app.fireBomb == False:
                hurtBoss(app,enemy,0)
        if (abs(app.player.cx-bomb.cx) < bomb.r+enemy.r
            and abs(app.player.cy-bomb.cy) < bomb.r+enemy.r):
            app.player.removeLives()
        app.displayExplosionUntil = app.fires+10
    if app.enemies == []:
        app.bomb.clear()

#hurts the boss according to its location with respect to the bomb
#if its holding the bomb, the bomb does not explode
#if its not holding the bomb, blows up bomb and carries out hit detection
def hurtBoss(app,enemy,r):
...
def explodeBomb(app,r):
    if app.bomb != []:
        bomb = app.bomb[0]
        if bomb.cy >= app.player.cy or r or hasBeenShot(app):
            bomb.r = 100
            app.fireBomb = False
            app.tempFire = None
            app.enemies[0].chargingBomb = False
            
#detects whether the bomb has been shot
#if it has, it blows up the bomb, and deletes the laser
def hasBeenShot(app):
    bomb = app.bomb[0]
    if bomb.r == 5:
        for laser in app.playerLasers:
            dist1 = (laser.x0-bomb.cx)**2+(laser.y0-bomb.cy)**2
            dist2 = (laser.x1-bomb.cx)**2+(laser.y0-bomb.cy)**2
            if dist1<25 or dist2<25:
                deleteLasers(app.playerLasers, app, laser)
                return True
    return False
...
def laserProcesses(app):
    moveLasers(app)
    deleteLasers(app.playerLasers,app,None)
    deleteLasers(app.enemyLasers,app,None)
...
#carries out the evasion if necessary
...
#checks whether the movement does not cause the enemy to move into
#the path of oncoming lasers
...
#changes the current heading of the enemy to an
#by changing it to an orthonormalized direction vector 
#based on the 3 rules of the boids algorithm 
#and one additional, added by me
...
#returns the vector component of the enemy's heading that
#ensures it follows the flockLeader
def followLeader(app,enemy):
    leader = app.flockLeader
    x = len(app.enemies)-1
    direction = [x*(leader.cx-enemy.cx)/50,x*(leader.cy-enemy.cy)/50]
    return direction

#ensures, as long as it is possible, the enemy's in the flock don't overlap
...
#ensures the enemy's move to the 'centre of mass' of all nearby enemies
#in a given range, ensuring cohesive movement
...
#checks whether the distance between two enemies is in a certain range
def isInRange(app,enemy,other):
    distance = math.sqrt((other.cx-enemy.cx)**2+(other.cy-enemy.cy)**2)
    return distance <= 4*enemy.r

#ensures the all the enemies move at roughly the same speed
