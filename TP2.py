...
def appStarted(app, r=6,stars = [], lasersFired = 0, hits = 0, lives = 5):
    app.timerDelay = 1
    app.gameOver = False
    app.victory = False
    app.proceed = False
    ...
    if not app.gameOver and not app.victory:
        if (app.enemyLasers != [] or app.enemies != []
            ...
            if app.player.lives <= 0:
                app.gameOver = True
            if (app.fires%1000 == 0 and app.round == 7
                and app.enemies[0].chargingBomb == False):
                app.enemies[0].chargingBomb = True
            if app.fires%1000 and app.round == 7:
                if app.enemies[0].chargingBomb == True:
                    chargeBomb(app)
                    if (app.bomb[0].r == 5
        ...
        for laser in app.playerLasers:
            ...
            if (leftBound-5<=laser.cx<=rightBound+5
                ...
                while not isLegalMove(app,moveX, moveY,enemy,enemy.r):
                    ...
                    moveX = 2*round(math.cos(moveAngle), 2)
                    moveY = 2*round(math.sin(moveAngle), 2)    
...
def isLegalMove(app,moveX,moveY,enemy,r):
    ...
    overlapAvoided = isNoOverlap(app,enemy,x,y)
    return (enemy.r<x<app.width-enemy.r
            and enemy.r<y<0.6*app.height-enemy.r 
            and moveX != 0 and moveY != 0 
            and shotAvoided and overlapAvoided)

def isNoOverlap(app,enemy,x,y):
    for otherEnemy in app.enemies:
        oX = otherEnemy.cx
        oY = otherEnemy.cy
        if (abs(oX-x) <= 2*enemy.r+1 and abs(oY-y) <= 2*enemy.r+1
